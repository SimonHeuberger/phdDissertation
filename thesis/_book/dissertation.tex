...
This section shows the imputation results for the MAR missing data mechanism. MAR amputation was achieved by setting the \texttt{mech} argument in the \texttt{ampute} function to \texttt{MAR}. Table \ref{mar.5var} shows the results of imputing all data sets MAR for five amputed variables. For the two binary variables, \texttt{Dem} and \texttt{Male}, \texttt{hd.ord} performs on par or worse than \texttt{hot.deck} for all data sets, while \texttt{mice} and \texttt{amelia} perform best. \texttt{hd.ord} is relatively close for CCES \texttt{Dem} (+.0001 \texttt{amelia} vs.~--.0004 \texttt{hd.ord}) but further away for ANES ( +.0003 \texttt{mice} vs.~--.0011 \texttt{hd.ord} \texttt{Dem}; +.0001 \texttt{mice} vs.~--.0013 \texttt{hd.ord} \texttt{Male}) and CCES \texttt{Male} (--.0001 \texttt{amelia} vs.~--.0014 \texttt{hd.ord}).
\begin{table}[!htbp] \centering 
...
This section shows the imputation results for the MNAR missing data mechanism. MNAR amputation was achieved by setting the \texttt{mech} argument in the \texttt{ampute} function to \texttt{MNAR}. All MAR and MNAR analyses are otherwise identical. Table \ref{mnar.5var} shows the results of imputing all data sets MNAR for five amputed variables. It is immediately noticable that the differences between the methods' imputation results and the true values is much higher for all methods for all variables for all data sets. The results for \texttt{Dem}, for instance, hover around .0100 for ANES/CCES and around .004 for Framing, while the \texttt{Male} numbers center around .0125 across all data sets. In the corresponding MAR analysis, however, the results for \texttt{Dem} and \texttt{Male} revealed around .0005 for all data sets.
\begin{table}[!htbp] \centering 
...
Table \ref{mult.mar.4var} shows the results of imputing all data sets with two \texttt{polr}-treated variables MAR for four amputed variables. \texttt{hd.ord} displays the worst results for \texttt{Dem} across all data sets and beats only \texttt{hot.deck} for \texttt{Male}. \texttt{amelia} and \texttt{mice} perform best and show virtually identically results that often match the true variable means. Comparison with the MAR analysis of five imputed variables reveals that \texttt{hd.ord} consistently performs slightly worse here: --.0018, --.0005, +.0012 vs.~--.0011, --.0004, +.0008 for \texttt{Dem} and --.0015, --.0018, +.0011 vs.~--.0013, --.0014, +.0005 for \texttt{Male}.
\begin{table}[!htbp] \centering 
