...
This leaves the non-numerical factor variables (e.g. `Gender`, `Race` etc.). Since an ANOVA test is not possible for these variables, I conduct a binomial GLM regression here. The results are shown in Table \ref{glm-sum}. Contrary to the numerical variables, a large number of the $p$-values for the variable intercepts are highly significant. This includes all intercepts of `Race`, `Income`, `Occupation`, `Party ID`, `Min. Wage`, and `Country Track`. \textcolor{red}{The only slight exceptions are `Gender` and `Pres. Approval`, where the intercepts for treatment group 1 (`Gender`) and treatment groups 2 and 3 (`Pres. Approval`) do not reach statistical significance.} The large number of statistically distinct categories indicates that a differentiation between education categories may be important for non-numerical factor variables. 
