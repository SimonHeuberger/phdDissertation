...
add.nas.columns <- c("Rep", "Ind", "Black", "Hisp", "Empl", "Stud", "Male", "Gay", "StudLoans",
                     "InternetHome", "Moderate", "NotReligious", "RentHome", "Separated", "inc", "interest")                            
...
# Any OPMord() results with fewer than min.lev get saved in OPMord.few. If there
# are no such results, OPMord.few = 0. Then nothing needs to be done. If there
# are such results, i.e. if OPMord.few \neq 0, then remove all NULL entries from
# OPMord.full, remove all NULL entries from OPMord.few, and save OPMord.few.
# Background: If OPMord.few = 0, the -which() stuff on OPMord.full would
# actually delete all entries, and the -which() stuff on OPMord.few would result
# in an error. Unless there are entries in OPMord.full AND OPMord.few (like in
# the ANES run), we need the if() around the -which() code
# if(length(OPMord.few) != 0){
#   OPMord.full <- OPMord.full[-which(sapply(OPMord.full, is.null))]
#   OPMord.few <- OPMord.few[-which(sapply(OPMord.few, is.null))]
#   if(run == "mine"){
#     saveRDS(OPMord.few, here("scripts", "testing", "om", paste0("OPM.ord.few.", length(add.nas.columns), "var.",
#                                                                 nrow(df_true), "n.", length(OPMord.few), "it.",
#                                                                 prop*100, "perc.rds")))
#   }
#   if(run == "jeff"){
#     saveRDS(OPMord.few, paste0("OPMord.few.", length(add.nas.columns), "var.",
#                                nrow(df_true), "n.", length(OPMord.few), "it.",
#                                prop*100, "perc.rds"))
#   }
#   if(run == "CO"){
#     saveRDS(OPMord.few, paste0("../results/OPMord.few.", length(add.nas.columns), "var.",
#                                nrow(df_true), "n.", length(OPMord.few), "it.",
#                                prop*100, "perc.rds"))
#   }
# }
