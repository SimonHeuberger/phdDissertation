...
for(n in 1:length(OPMord.full)){
  ...
  # am.temp <- amelia(OPMord.full[[n]]$data.full.nas, m = m, p2s = 0)
  # if(am.temp$message == "One or more of the imputations resulted in a covariance matrix that was not invertible."){
  #   am.temp.2 <- amelia(OPMord.full[[n]]$data.full.nas, m = m, p2s = 2)
  #   if(am.temp.2$message == "One or more of the imputations resulted in a covariance matrix that was not invertible."){
  #     am.orig.dat[[n]] <- amelia(OPMord.full[[n]]$data.full.nas, m = m, p2s = 2)
  #   }else{
  #     am.orig.dat[[n]] <- am.temp.2
  #   }
  # }else{
