...
if(run == "mine"){
  ...
  df_true <- readRDS(here("data", "cces", "cces_1000.rds")) # I took a sample of 1000 rows from the CCES data in the separate cces script, making sure it contains all educ levels, then I saved that sample
}
if(run == "jeff"){
  load("OPMordMult.Rdata") 
  load("OPMcutMult.Rdata") 
  load("hot.deck.ord.mult.Rdata")
  df_true <- readRDS("cces_1000.rds") 
}
if(run == "CO"){
  load("../data/OPMordMult.Rdata") 
  load("../data/OPMcutMult.Rdata") 
  load("../data/hot.deck.ord.mult.Rdata")
  df_true <- readRDS("../data/cces_1000.rds")
}
...
add.nas.columns <- c("Rep", "Ind", "Black", "Hisp", "Empl", "Stud", "Male", "Gay", "StudLoans",
                     "InternetHome", "Moderate", "NotReligious", "RentHome", "Separated", "inc")     
...
for(n in 1:length(OPMord.full)){
  setTxtProgressBar(pb, n)
  # run hot.deck.ord.mult() on OPMcut output
