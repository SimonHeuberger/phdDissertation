...
if(run == "mine"){
  library(here)
  load(here("functions", "OPMordMult.Rdata"))
  load(here("functions", "OPMcutMult.Rdata")) 
  load(here("functions", "hot.deck.ord.mult.Rdata"))
  df_true <- readRDS(here("data", "anes", "anes_1000.rds")) # I took a sample of 1000 rows from the ANES data in the separate ANES script, then I saved that sample. It contains fewer levels, there is no way to get all
}
if(run == "jeff"){
  load("OPMordMult.Rdata") 
  load("OPMcutMult.Rdata") 
  load("hot.deck.ord.mult.Rdata")
  df_true <- readRDS("anes_1000.rds") 
}
if(run == "CO"){
  load("../data/OPMordMult.Rdata") 
  load("../data/OPMcutMult.Rdata") 
  load("../data/hot.deck.ord.mult.Rdata")
  df_true <- readRDS("../data/anes_1000.rds")
}

df_true <- mutate(df_true, educ = ifelse(educ == 1 | 
                                               educ == 2 | 
                                               educ == 3 | 
                                               educ == 4 | 
                                               educ == 5 | 
...
# save name of DV and all EVs
dvs <- c("educ", "interest")
all.evs <- colnames(df_true)[-which(colnames(df_true) %in% dvs)]                         

# variables to insert NAs into
add.nas.columns <- c("Rep", "Ind", "Black", "Hisp", "Empl", "Stud", "Male", "VotePres2016", "CongressJob",
                     "CountryTrack", "HealthIns", "GlobWarm", "Religious", "InternetHome", "RentHome", "Rally",
                     "Donate", "Liberal", "Moderate", "Separated", "inc")                            
...
# the complete number of levels of the dvs
min.levs <- sapply(df_true[, dvs], unique) %>% sapply(., length)
...
for(mc in 1:mc.iterations){
  ...
  # extract levels of ordinal variables in data.amp
  ord.lev <- na.omit(data.amp) %>% 
    .[, dvs] %>% 
    sapply(., unique) %>% 
    sapply(., length)
  if(TRUE %in% (ord.lev < min.levs)){
    OPMord.few[[mc]] <- OPMordMult(data.amp, 
                                   dv = dvs, 
                                   evs = all.evs,
                                   NAcols = add.nas.columns)
  }else{
    OPMord.full[[mc]] <- OPMordMult(data.amp, 
                                    dv = dvs, 
                                    evs = all.evs,
                                    NAcols = add.nas.columns)
  }
...
for(n in 1:length(OPMord.full)){
  ...
  OPMcut.dat[[n]] <- OPMcutMult(data = OPMord.full[[n]]$data.full.nas,
                                dv = dvs,
                                NAcols = add.nas.columns,
                                OPMordMultOut = OPMord.full[[n]])
}
...
for(n in 1:length(OPMord.full)){
  ...
  hd.ord.dat[[n]] <- hot.deck.ord.mult(OPMcut.dat[[n]], 
                                       m = m, 
                                       ord = dvs,
...
for(n in 1:length(OPMord.full)){
  ...
  am.orig.dat[[n]] <- amelia(OPMord.full[[n]]$data.full.nas, 
                             m = m, 
                             p2s = 0)
}
end_time <- Sys.time()
...
for(n in 1:length(OPMord.full)){
  ...
  hd.norm.orig.dat[[n]] <- hot.deck(OPMord.full[[n]]$data.full.nas, 
                                    m = m,
...
for(n in 1:length(OPMord.full)){
  ...
  mice.orig.dat[[n]] <- mice(OPMord.full[[n]]$data.full.nas,
                             m = m, 
                             print = FALSE)
}
end_time <- Sys.time()
