...
df <- rename(df, age = birthyr, gender = gender, race = race, income = faminc, occupation = employ,
             pid = pid3, education = educ, interest = newsint, sexuality = sexuality, studLoans = edloan,
             internetHome = internethome, ideology = ideo5, religion = religpew, ownHome = ownhome, 
             marStat = marstat)
nrow(df) # 52393 observations

# remove observations with refused, missing etc.
df <- filter(df, !income %in% c("Prefer not to say"),
             !pid %in% c("Not sure"),
...
df <- mutate(df,inc = ifelse(income == "Less than $10,000", 1,
                      ifelse(income == "$10,000 - $19,999", 2,
                      ifelse(income == "$20,000 - $29,999", 3,
                      ifelse(income == "$30,000 - $39,999", 4,
                      ifelse(income == "$40,000 - $49,999", 5,
                      ifelse(income == "$50,000 - $59,999", 6,
                      ifelse(income == "$60,000 - $69,999", 7,
                      ifelse(income == "$70,000 - $79,999", 8,
                      ifelse(income == "$80,000 - $99,999", 9,
                      ifelse(income == "$100,000 - $119,999", 10,
                      ifelse(income == "$120,000 - $149,999", 11, 12))))))))))))

df <- mutate(df,educ = ifelse(education == "No HS", 1,
                       ifelse(education == "High school graduate", 2,
                       ifelse(education == "Some college", 3,
                       ifelse(education == "2-year", 4,
                       ifelse(education == "4-year", 5, 6))))))
...
df_true <- df[, c("Dem", "Rep", "Ind", "Black", "Hisp",
                  ...
                  "Ret", "Stud", "Straight", "Gay", "Bisexual", "StudLoans",
...
# set.seed(130)
# cces_1000 <- sample(nrow(df_true), 1000, replace = TRUE) %>% df_true[.,]
