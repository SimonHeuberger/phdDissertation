...
add.nas.columns <- c("Dem", "Male", "Inc", "Age", "Black", "Empl", 
                     "Religious", "Married", "OwnHome", "Rally", "Donate")
# imputation methods
...
df_true <- df_true[,!names(df_true) %in% c("Liberal", "Conservative", "Separated", "Single")] # collinearity too high
nrow(df_true)
...
for(n in 1:length(OPMord.full)){
  ...
  # if(am.temp$message == "One or more of the imputations resulted in a covariance matrix that was not invertible."){
  #   am.orig.dat[[n]] <- amelia(OPMord.full[[n]]$data.full.nas, m = m, p2s = 2)
  # }else{
  #   am.orig.dat[[n]] <- am.temp
  # }
}
end_time <- Sys.time()
...
for(n in 1:length(OPMord.full)){
  ...
  hot.deck.dat[[n]] <- hot.deck(OPMord.full[[n]]$data.full.nas, 
                                    m = m,
