...
df <- rename(df, age = V161267, gender = V161342, race = V161310x, income =  V161361x, occupation = V161276x, pid = V161155, education = V161270, interest = V162257)
...
df <- mutate(df, interest = ifelse(interest == "1. Very closely", 1,
                            ifelse(interest == "2. Fairly closely", 2,
                            ifelse(interest == "3. Not very closely", 3, 4))))
...
# store OPMord output
df.nas <- lapply(1:mc.iterations, function(x)cbind(no.nas, ampute(yes.nas, prop = prop, mech = "MAR", cont = FALSE)$amp))
OPMord.dat <- lapply(1:mc.iterations, function(x)OPMord(data = df.nas[[x]], dv = "educ", evs = all.evs))
...
OPMcut.dat <- lapply(1:length(OPMord.dat), function(x)OPMcut(data = OPMord.dat[[x]]$data.full.nas,                                  
                           dv = "educ", OPMordOut = OPMord.dat[[x]]))


start_time <- Sys.time()
hd.ord.dat <- lapply(1:length(OPMord.dat), function(x)hot.deck.ord(OPMcut.dat[[x]], m = m, ord = "educ", 
                                 sdCutoff = sd(OPMcut.dat[[x]]$educ), method = "best.cell"))
end_time <- Sys.time()
hd.ord.time <- difftime(end_time, start_time, units = "mins")


start_time <- Sys.time()
hd.norm.orig.dat <- lapply(1:length(OPMord.dat), function(x)hot.deck(OPMord.dat[[x]]$data.full.nas, m = m,                          
                                   sdCutoff = sd(OPMord.dat[[x]]$data.full.nas$educ), 
                                   method = "best.cell"))
end_time <- Sys.time()
...
start_time <- Sys.time()
am.orig.dat <- lapply(1:length(OPMord.dat), function(x)amelia(OPMord.dat[[x]]$data.full.nas, m = m, p2s = 0))
end_time <- Sys.time()
am.time <- difftime(end_time, start_time, units = "mins")


start_time <- Sys.time()
mice.orig.dat <- lapply(1:length(OPMord.dat), function(x)mice(OPMord.dat[[x]]$data.full.nas, m = m, print = FALSE))
end_time <- Sys.time()
