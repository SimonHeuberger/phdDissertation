...
# df <- data.frame(matrix(NA, 1000, 5))
# df[,1] <- sample(7, 1000, replace = TRUE)
# df[,2] <- sample(0:1, 1000, replace = TRUE)
# df[,3] <- sample(3, 1000, replace = TRUE)
# df[,4] <- sample(4, 1000, replace = TRUE)
# df[,5] <- sample(10, 1000, replace = TRUE)
# 
# colnames(df) <- c("income", "Dem", "Male", "interest", "age")
# # colDep = "income"
# # colNA = "age"
# 
# 
# ff <- function(df, prop, colDep, colNA){
#   df.orig <- df
#   df$id <- as.numeric(rownames(df))
#   uqs <- unique(df[, colDep])
#   for(n in 1:length(uqs)){
#       ids <- df[which(df[, colDep] == uqs[n]), ] %>% .$id %>% sample(., round(length(.)*prop))
#       df[, colNA][df$id %in% ids] <- NA
#   }
#   return(list(df = df, df.orig = df.orig))
# }
# 
# 
# rr <- ff(df = df, prop = 0.2, colDep = "income", colNA = "age")
# 
# 
# 
# 
# colDep = c("income", "Dem")
# colNA = c("interest", "age")
...
add.nas.columns <- c("Dem", "age", "interest")
# variables to base NAs onto
colDep <- c("inc", "Female", "Black")
...
pb <- txtProgressBar(min = 1, max = mc.iterations, style = 3)                                       
  ...
  for(mc in 1:mc.iterations){
    ...
    list.own.NA[[mc]] <- own.NA(df = df_true, prop = prop, colDep = colDep, colNA = add.nas.columns)
