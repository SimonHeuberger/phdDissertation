...
add.nas.columns <- c("Dem", "Male", "Inc", "Age")
# imputation methods
...
df_true <- df_true[,!names(df_true) %in% c("Liberal", "Conservative", "Single")] # collinearity too high
nrow(df_true)
...
# extract EVs
all.evs <- colnames(df_true)[-which(colnames(df_true) %in% dv)]                         
...
min.levs <- sapply(df_true[, dv], unique) %>% sapply(., length)
