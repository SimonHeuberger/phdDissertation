...
if(run == "mine"){
  ...
  df_true <- read.csv(here::here("data", "experiment", "an.clean.csv"), stringsAsFactors = TRUE, check.names = FALSE)
}
if(run == "jeff"){
  load("OPMord.Rdata") 
  load("OPMcut.Rdata") 
  load("hot.deck.ord.Rdata") 
  df_true <- read.csv("an.clean.csv", stringsAsFactors = TRUE, check.names = FALSE)
}
if(run == "CO"){
  load("../data/OPMord.Rdata") 
  load("../data/OPMcut.Rdata") 
  load("../data/hot.deck.ord.Rdata")
  df_true <- read.csv("../data/an.clean.csv", stringsAsFactors = TRUE, check.names = FALSE)
}
...
df_true <- mutate(df_true,
                  ...
                  Educ = ifelse(educ == "Up to 1st grade" |     # needs to be reduced to 6 levels, it will never work with all these levels
                                educ == "1st-4th grade" |
                                educ == "5th-6th grade" |
                                educ == "7th-8th grade" |
                                educ == "9th grade" |
