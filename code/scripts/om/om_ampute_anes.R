...
df <- rename(df, Age = V161267, gender = V161342, race = V161310x, Inc =  V161361x, occupation = V161276x, pid = V161155,
             Educ = V161270, Interest = V162257, votePres2016 = V161026, congressHandleJob = V161080, countryRightTrack = V161081,
...
df <- filter(df, 
             ...
             # do NOT include those 5 above!!!
             # that will massively drop the number of observations to just 85!!!
...
df <- mutate(df,Inc = ifelse(Inc == "01. Under $5,000", 1,
                      ifelse(Inc == "02. $5,000-$9,999", 2,
                      ifelse(Inc == "03. $10,000-$12,499", 3,
                      ifelse(Inc == "04. $12,500-$14,999", 4,
                      ifelse(Inc == "05. $15,000-$17,499", 5,
                      ifelse(Inc == "06. $17,500-$19,999", 6,
                      ifelse(Inc == "07. $20,000-$22,499", 7,
                      ifelse(Inc == "08. $22,500-$24,999", 8,
                      ifelse(Inc == "09. $25,000-$27,499", 9,
                      ifelse(Inc == "10. $27,500-$29,999", 10,
                      ifelse(Inc == "11. $30,000-$34,999", 11,
                      ifelse(Inc == "12. $35,000-$39,999", 12,
                      ifelse(Inc == "13. $40,000-$44,999", 13,
                      ifelse(Inc == "14. $45,000-$49,999", 14,
                      ifelse(Inc == "15. $50,000-$54,999", 15,
                      ifelse(Inc == "16. $55,000-$59,999", 16,
                      ifelse(Inc == "17. $60,000-$64,999", 17,
                      ifelse(Inc == "18. $65,000-$69,999", 18,
                      ifelse(Inc == "19. $70,000-$74,999", 19,
                      ifelse(Inc == "20. $75,000-$79,999", 20,
                      ifelse(Inc == "21. $80,000-$89,999", 21,
                      ifelse(Inc == "22. $90,000-$99,999", 22,
                      ifelse(Inc == "23. $100,000-$109,999", 23,
                      ifelse(Inc == "24. $110,000-$124,999", 24,
                      ifelse(Inc == "25. $125,000-$149,999", 25,
                      ifelse(Inc == "26. $150,000-$174,999", 26, 27)))))))))))))))))))))))))))
                                                                                                                                                        
df <- mutate(df, Educ = ifelse(Educ == "1. Less than 1st grade" | 
                                         Educ == "2. 1st, 2nd, 3rd or 4th grade" | 
                                         Educ == "3. 5th or 6th grade" | 
                                         Educ == "4. 7th or 8th grade" | 
                                         Educ == "5. 9th grade" | 
                                         Educ == "6. 10th grade" | 
                                         Educ == "7. 11th grade" | 
                                         Educ == "8. 12th grade no diploma" | 
                                         Educ == "9. High school graduate- high school diploma or equivalent (for example: GED)", 1,
                                         ifelse(Educ == "10. Some college but no degree", 2,
                                         ifelse(Educ == "11. Associate degree in college - occupational/vocational program" |
                                                Educ == "12. Associate degree in college -- academic program", 3,
                                         ifelse(Educ == "13. Bachelor's degree (for example: BA, AB, BS)", 4,
                                         ifelse(Educ == "14. Master's degree (for example: MA, MS, MENG, MED, MSW, MBA)", 5, 6))))))
...
df_true <- df[, c("Dem", "Ind", "Black", "Hisp",
                  "White", "Asian", "Male", "Empl",        
                  #"Ret", # collinear with Age
                  #"Rep", # collinear with Dem
                  #"Female", # collinear with Male
                  "Stud", "Interest", "Inc", "Age", "Educ",
                  "Religious", "InternetHome", "OwnHome",
                  #"RentHome", # collinear with OwnHome
                  "Rally", "Donate", "Liberal", "Conservative",
                  "Moderate", "Married", "Separated", "Single")]  


# save the data for when I want to run things for all observations
# saveRDS(df_true, here("data", "anes", "anes_all.rds"))


# take a sample of 1000 observations, then save it, to try to get around the CO RAM collapses
# set.seed(160)
# df_true$Educ %>% unique %>% length
# anes_1000 <- sample(nrow(df_true), 1000) %>% df_true[.,]
