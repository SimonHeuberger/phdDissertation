...
dataaa <- readRDS(here("data", "anes", "anes_cleaned.rds")) 
dv <- c("education", "income")
evs <- c("age", "gender", "race", "occupation", "pid")
fg <- OPMordMult(data = dataaa, dv = dv, evs = evs)
data <- OPMcutMult(data = dataaa, dv = dv, OPMordMultOut = fg)
m = 5
method = "best.cell"
cutoff = 20
sdCutoff = 1
...
function(data, ord, m = 5, method=c("best.cell", "p.draw"), cutoff=10, sdCutoff=1, optimizeSD = FALSE,   # adds "ord" to the list
  ...
  impContinuous <- match.arg(impContinuous)
	...
	tmp <- scaleContinuous(data, alldisc, sdx=1/sdCutoff)
	ord.disc <- FALSE  # added by me for scaleContinuous for all ordinal variables
	for(i in 1:length(ord)){ # added by me for scaleContinuous for all ordinal variables
